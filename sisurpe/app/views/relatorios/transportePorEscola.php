<?php
  ...
  $colunas =array("N","Nome do Aluno","Sexo","Transporte1","Transporte2","Transporte3");
  //largura das colunas
  ...
  if($data['escola']){ 
    ...
    $pdf->Cell(50, 5, utf8_decode('Relatório: Transporte por Escola'), 0, 1, 'L');
    $pdf->Ln();
    $pdf->SetFont('Arial','B',12);
    ...
    foreach($data['turmas'] as $key => $row){
      ...
      if($row['coleta']){
        ...
        foreach($row['coleta'] as $coleta){           
          ...
          $pdf->Cell($larguracoll[4],5,utf8_decode($coleta->transporte1),1,0,'C');
          $pdf->Cell($larguracoll[5],5,utf8_decode($coleta->transporte2),1,0,'C');
          $pdf->Cell($larguracoll[6],5,utf8_decode($coleta->transporte3),1,0,'C');
          $pdf->Ln();                       
        }   
        $pdf->SetFont('Arial','B',7);
        $pdf->Cell(25,5,utf8_decode('Totais'),1,0,'C');  
				$countT = 1;
        foreach($row['totaisTransporte'] as $key => $total){ 
            if($countT % 7 == 0){
                $pdf->Ln();
            }
            $pdf->Cell(25,5,utf8_decode('('.$key . '): ' . $total),1,0,'C');
						$countT++;
        }   
      } else {
		...
		$pdf->Ln();
		$pdf->Ln();
    $pdf->Cell(25,5,utf8_decode('Totais Gerais'),1,0,'C');    
    foreach($data['totalGeral'] as $key => $total){ 
			if($key >0 && $key % 6 == 0){
        $pdf->Ln();
      }    
      $pdf->Cell(25,5,utf8_decode('('. $total['linha'] . '): ' . $total['total']),1,0,'C');
    } 
		/* totais da escola */     
  } else {
