<?php
	class Fuseroutroscurso {
		...
		public function getUserOutrosCursos($_userId){            
			$this->db->query("
				SELECT 
					fuoc.userId as userId, 
					fuoc.cursoId as cursoId, 
					foc.curso as curso 
				FROM 
					f_user_outros_cursos fuoc, 
					f_outros_cursos foc 
				WHERE 
					fuoc.cursoId = foc.cursoId 
				AND 
					fuoc.userId = :userId
			");
		...
		public function register($data,$_userId){				
			...
			if($data){
				foreach($data as $row){
					$this->db->query("
						INSERT INTO f_user_outros_cursos SET userId = :userId, cursoId = :cursoId
					");
					$this->db->bind(':userId',$_userId);
